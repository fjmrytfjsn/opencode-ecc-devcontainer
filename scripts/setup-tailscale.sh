...
if pgrep -f tailscaled > /dev/null; then
    ...
    if sudo tailscale --socket=/run/tailscale/tailscaled.sock status > /dev/null 2>&1; then
        CURRENT_IP=$(sudo tailscale --socket=/run/tailscale/tailscaled.sock ip -4 2>/dev/null || echo "未接続")
        ...
        sudo tailscale --socket=/run/tailscale/tailscaled.sock down || true
        sleep 2
    fi
else
...
if [[ -f "$ENV_FILE" ]]; then
    ...
    if is_valid_tailscale_key "$EXISTING_KEY"; then
        ...
        echo -e "${CYAN}Auth Keyを更新する場合のみ入力してください（Enterで既存キーを維持）:${NC}"
...
if [[ -z "$AUTH_KEY" ]]; then
    ...
    # Auth Key 形式確認
    if ! is_valid_tailscale_key "$AUTH_KEY"; then
...
if [[ -f "$ENV_FILE" ]]; then
    upsert_env_value "$ENV_FILE" "TAILSCALE_AUTH_KEY" "$AUTH_KEY"
    upsert_env_value "$ENV_FILE" "TAILSCALE_HOSTNAME" "$HOSTNAME"
...
if ! pgrep -f tailscaled > /dev/null; then
    ...
    sudo tailscaled --statedir=/var/lib/tailscale --socket=/run/tailscale/tailscaled.sock --tun=userspace-networking --socks5-server=localhost:1055 >/tmp/tailscaled.log 2>&1 &
    sleep 3
fi

# Tailscale認証
echo "   Auth Key で認証中..."
if sudo tailscale --socket=/run/tailscale/tailscaled.sock up --auth-key="$AUTH_KEY" --hostname="$HOSTNAME" --accept-routes; then
    echo -e "${GREEN}✅ Tailscale接続成功！${NC}"
    
    # IP取得
    sleep 2
    TAILSCALE_IP=$(sudo tailscale --socket=/run/tailscale/tailscaled.sock ip -4 2>/dev/null || echo "IP取得中...")
    echo -e "${GREEN}   📱 Tailscale IP: $TAILSCALE_IP${NC}"
    
    # 接続状態確認
    echo ""
    echo -e "${BLUE}🌐 接続状態確認:${NC}"
    sudo tailscale --socket=/run/tailscale/tailscaled.sock status | head -5
